/* esm.sh - github:haskell-wasm/browser_wasi_shim#2f86b49 */
...
// node_modules/haskell-wasm/browser_wasi_shim/src/wasi_defs.ts
...
var Filestat = class {
  ...
  mtim;
  ctim;
  constructor(ino, filetype, size, atim = 0n, mtim = 0n, ctim = 0n) {
    this.ino = ino;
...
// node_modules/haskell-wasm/browser_wasi_shim/src/debug.ts
var Debug = class {
...
// node_modules/haskell-wasm/browser_wasi_shim/src/fd.ts
var Fd = class {
...
// node_modules/haskell-wasm/browser_wasi_shim/src/fs_mem.ts
...
var OpenDirectory = class extends Fd {
  ...
  path_filestat_set_times(_flags, path_str, atim, mtim, fst_flags) {
    const { ret: path_err, path } = Path.from(path_str);
    if (path == null) {
      return path_err;
    }
  ...
  path_symlink(old_path_str, new_path_str) {
...
var InodeMem = class extends Inode {
  atim;
  mtim;
  ctim;
  constructor() {
  ...
  set_times(atim, mtim, fst_flags) {
    const now = realtime();
    if (fst_flags & FSTFLAGS_ATIM) {
      this.atim = atim;
      this.ctim = now;
    }
    if (fst_flags & FSTFLAGS_ATIM_NOW) {
      this.atim = now;
      this.ctim = now;
    }
    if (fst_flags & FSTFLAGS_MTIM) {
      this.mtim = mtim;
      this.ctim = now;
    }
    if (fst_flags & FSTFLAGS_MTIM_NOW) {
      this.mtim = now;
      this.ctim = now;
    }
    return ERRNO_SUCCESS;
...
var File = class extends InodeMem {
  data;
  readonly;
...
var Directory = class _Directory extends InodeMem {
  contents;
  parent = null;
  pendingSymlinkTargets;
  ...
  registerPendingLinkTarget(name, pending) {
    if (this.pendingSymlinkTargets == void 0) {
      this.pendingSymlinkTargets = /* @__PURE__ */ new Map();
    }
    const pendingList = this.pendingSymlinkTargets.get(name);
    if (pendingList == void 0) {
      this.pendingSymlinkTargets.set(name, [pending]);
    } else {
      pendingList.push(pending);
    }
  }
  removePendingLinkTarget(name, pending) {
    const pendingList = this.pendingSymlinkTargets?.get(name);
    if (pendingList == void 0) {
      return;
    }
  ...
  resolvePendingLinkTarget(name, entry) {
    const pendingList = this.pendingSymlinkTargets?.get(name);
    if (pendingList == void 0) {
      return;
    }
...
var PendingSymlink = class extends InodeMem {
  parent;
  filename;
  targetDir;
  targetName;
  constructor(parent, filename, targetDir, targetName) {
    super();
    this.parent = parent;
...
function detachPendingSymlink(entry) {
...
// node_modules/haskell-wasm/browser_wasi_shim/src/wasi.ts
...
// node_modules/haskell-wasm/browser_wasi_shim/src/fs_opfs.ts
...
// node_modules/haskell-wasm/browser_wasi_shim/src/strace.ts
...
//# sourceMappingURL=browser_wasi_shim.development.mjs.map
